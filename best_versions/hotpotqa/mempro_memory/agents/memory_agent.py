...
class MemoryAgent:
    ...
    def _decorate(self, message: str, memory_state: MemoryState) -> Tuple[str, str, str]:
        ...
        base_url = getattr(self.generator, "base_url", "<unknown>")
        print(f"[MemoryAgent] generating abstract with {generator_name} model={model_name} base_url={base_url}")
        print(f"[MemoryAgent] prompt head: {prompt[:500]}")

        response = self.generator.generate_single(prompt=prompt)
        abstract = response.get("text", "").strip()
        print(f"[MemoryAgent] raw abstract head: {abstract[:300]}")
        if not abstract:
            raise RuntimeError("Empty abstract returned by generator")
